...
public class IntervalReadingEntity implements Serializable {
    ...
    /**
     * Adds a reading quality indicator to this interval reading.
    ...
    /**
     * Removes a reading quality indicator from this interval reading.
    ...
    public void removeReadingQuality(ReadingQualityEntity readingQuality) {
        if (readingQuality != null) {
            this.readingQualities.remove(readingQuality);
            readingQuality.setIntervalReading(null);
        }
    }

    /**
     * Sets the interval block for this reading.
     * This method ensures the bidirectional relationship is maintained.
     * 
     * @param intervalBlock the interval block to set
     */
    public void setIntervalBlock(IntervalBlockEntity intervalBlock) {
        // Remove from old interval block if exists
        if (this.intervalBlock != null && this.intervalBlock != intervalBlock) {
            this.intervalBlock.removeIntervalReading(this);
        }
        
        this.intervalBlock = intervalBlock;
        
        // Add to new interval block if not null
        if (intervalBlock != null && !intervalBlock.getIntervalReadings().contains(this)) {
            intervalBlock.addIntervalReading(this);
        }
    }
    ...
    public void merge(IntervalReadingEntity other) {
        if (other != null) {
            ...
            // Replace reading qualities
            this.readingQualities.clear();
            if (other.readingQualities != null) {
                for (ReadingQualityEntity quality : other.readingQualities) {
                    addReadingQuality(quality);
                }
            }
     ...
     */
    public void unlink() {
        // Clear reading qualities
        for (ReadingQualityEntity quality : new ArrayList<>(readingQualities)) {
            removeReadingQuality(quality);
        }
