...
public class SubscriptionEntity extends IdentifiedObjectEntity {
    ...
    /**
     * Sets the authorization for this subscription.
     * This method ensures the bidirectional relationship is maintained.
     * 
    ...
    public void setAuthorization(AuthorizationEntity authorization) {
        ...
        if (authorization != null) {
            authorization.setSubscription(this);
        }
    }

    /**
     * Adds a usage point to this subscription.
    ...
    public void addUsagePoint(UsagePointEntity usagePoint) {
        if (usagePoint != null && !this.usagePoints.contains(usagePoint)) {
            this.usagePoints.add(usagePoint);
            // Note: UsagePointEntity might have a subscriptions collection - we'll handle that when we create it
        }
    }

    /**
     * Removes a usage point from this subscription.
    ...
    /**
     * Removes all usage points from this subscription.
     * Clears all usage point relationships.
     */
    public void clearUsagePoints() {
        // Create a copy to avoid concurrent modification
        List<UsagePointEntity> usagePointsCopy = new ArrayList<>(this.usagePoints);
        for (UsagePointEntity usagePoint : usagePointsCopy) {
    ...
    public void unlink() {
        ...
        // Clear authorization (this will also clear the bidirectional relationship)
        if (authorization != null) {
     ...
     * @return true if included, false otherwise
     */
    public boolean includesUsagePointId(Long usagePointId) {
        if (usagePoints == null || usagePointId == null) {
            return false;
        }
        return usagePoints.stream()
            .anyMatch(up -> usagePointId.equals(up.getId()));
    }
     ...
     */
    public boolean isForApplication(Long applicationId) {
        return applicationInformation != null && applicationId != null && 
               applicationId.equals(applicationInformation.getId());
    }
