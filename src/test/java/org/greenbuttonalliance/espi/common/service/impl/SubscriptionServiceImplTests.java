...
public class SubscriptionServiceImplTests extends BaseTest {
	 ...
	 * start of comment
	 * 
	 * @Before public void before() { service = new SubscriptionServiceImpl();
	 * applicationInformationService = new ApplicationInformationServiceImpl();
	 * repository = mock(SubscriptionRepositoryImpl.class); retailCustomer =
	 * newRetailCustomer(); oAuth2Request =
	 * newOAuth2Request("third_party_client");
	 ...
	 * .findByClientId(oAuth2Request.getClientId
	 ...
	 * 
	 * @Test public void createSubscription_persistsSubscription() {
	 * verify(repository).persist(eq(subscription)); }
	 * 
	 * @Test public void createSubscription_setsRetailCustomer() {
	 * assertEquals(retailCustomer, subscription.getRetailCustomer()); }
	 * 
	 * @Test public void createSubscription_setsUUID() {
	 * assertNotNull(subscription.getUUID()); }
	 * 
	 * @Test public void createSubscription_setsLastUpdateToEpoch() {
	 * assertThat(subscription.getLastUpdate(), is(DateConverter.epoch())); }
	 * 
	 * @Test public void findAll() { List<Subscription> subscriptions = new
	 * ArrayList<>();
	 * 
	 ...
	 * 
	 * @Test public void findEntriesByHashedId() throws Exception { List<Long>
	 * ids = Lists.newArrayList(1L);
	 * 
	 * when(repository.findByHashedId(subscription.getHashedId())).thenReturn(
	 * subscription);
	 * when(usagePointService.findAllIdsForRetailCustomer(subscription
	 * .getRetailCustomer().getId())).thenReturn(ids);
	 * 
	 * EntryTypeIterator entries =
	 * service.findEntriesByHashedId(subscription.getHashedId());
	 * 
	 * assertThat(entries, is(not(nullValue()))); assertThat(entries.hasNext(),
	 * is(true)); } End of Comment
	 */
}
